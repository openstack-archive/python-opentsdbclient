...
class RESTOpenTSDBClient(client.BaseOpenTSDBClient):
    ...
    def get_statistics(self):
        ...
        req = requests.get(utils.STATS_TEMPL % {'host': self.hosts[0],
                                                'port': self.port})
        return req
    ...
    def put_meter(self, meters):
        ...
        for meter_dict in meters:
            req = requests.post(utils.PUT_TEMPL %
                                {'host': self.hosts[0], 'port': self.port},
    ...
    def define_retention(self, tsuid, retention_days):
        ...
        req = requests.post(utils.META_TEMPL % {'host': self.hosts[0],
                                                'port': self.port,
                                                'tsuid': tsuid},
    ...
    def get_aggregators(self):
        ...
        req = requests.get(utils.AGGR_TEMPL % {'host': self.hosts[0],
                                               'port': self.port})
        return req
    ...
    def get_version(self):
        ...
        """
        req = requests.get(utils.VERSION_TEMPL % {'host': self.hosts[0],
                                                  'port': self.port})
        return req
    ...
    def _make_query(self, query, verb):
        ...
        if meth is None:
            pass
        req = meth(utils.QUERY_TEMPL % {'host': self.hosts[0],
                                        'port': self.port,
                                        'query': query})
        return req
