...
class SocketOpenTSDBClient(client.BaseOpenTSDBClient):

    def __init__(self, opentsdb_hosts, opentsdb_port,
                 conn_verify_trusted_time=60, reconnect_interval=0,
                 send_queue_max_size=1000, max_uncaught_exceptions=100):
        super(client.BaseOpenTSDBClient, self).__init__(opentsdb_hosts,
                                                        opentsdb_port)
        self.send_queue = []
        self.tsd = None
        self.host = None
    ...
    def pick_connection(self):
        ...
        for self.current_tsd_host in xrange(self.current_tsd_host + 1,
                                            len(self.hosts)):
    ...
    def compose_line_from_metrer(self, meter_dict):
    ...
    def send_data(self):
        req = "\n".join("put %s" % self.compose_line_from_metrer(meter_dict)
